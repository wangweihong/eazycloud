...
import (
	...
	"github.com/wangweihong/eazycloud/pkg/cache"
...
func (u userManager) ListInTenant(tenant string) []*User {
	...
	if err != nil {
		log.Errorf("index with indexer %v err:%v", indexTypeTenantUser, err)
		return users
	}
	for _, v := range objects {
		if u, ok := v.(*User); ok {
...
func (u userManager) ListInGroup(groups string) []*User {
	...
	if err != nil {
		log.Errorf("index with indexer %v err:%v", indexTypeGroupUser, err)
		return users
	}
	for _, v := range objects {
		if u, ok := v.(*User); ok {
	...
	for _, v := range objects {
		if user, ok := v.(*User); ok {
			...
			if err := u.Indexer.Update(meta); err != nil {
				log.Error("update user err", log.Any("user", meta), log.Err(err))
			}
		}
	}
	return nil
...
func (u userManager) ListInRole(role string) []*User {
	...
	if err != nil {
		log.Errorf("index with indexer %v err:%v", indexTypeRoleUser, err)
		return users
	}
	for _, v := range objects {
		if u, ok := v.(*User); ok {
